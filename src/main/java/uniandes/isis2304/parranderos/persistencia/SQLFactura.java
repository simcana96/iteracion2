...
public class SQLFactura {
	...
	{
		 Query q = pm.newQuery(SQL, "INSERT INTO "+ps.darFactura ()   + "(ID_FACTURA, FECHA_FACTURA, ID_CLIENTE) values (?, ?, ?)");
	...
	{
		Query q = pm.newQuery(SQL, "SELECT * FROM " + ps.darFactura());
	...
	{
		Query q = pm.newQuery(SQL, "SELECT * FROM " + ps.darFactura() + " WHERE ID_FACTURA = ?");
		q.setResultClass(VOFactura.class);
	...
	{
		Query q = pm.newQuery(SQL, "SELECT * FROM "+ps.darFactura() + " WHERE ID_CLIENTE = ?");
		q.setResultClass(VOFactura.class);
